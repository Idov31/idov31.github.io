...
export default function StyledLink({href, content, textSize = "text-md"}: StyledLinkProps) {
    return (
        <Link
            className={`text-txtLink ${textSize} hover:underline transition-colors duration-200`}
            href={href}
        >
            {content}
        </Link>
...
export function StyledHeaderLink({href, content}: StyledHearderLinkProps) {
    return (
        <Link
            className="text-txtHeader hover:text-txtSubHeader transition-colors duration-200"
            href={href}
        >
            {content}
        </Link>
...
export function StyledBarLink({href, content, mr = 2, isBurger = false}: StyledBarLinkProps) {
    return (
        <Link
            className={`mb-2 mr-${mr} ${isBurger ? "text-2xl" : "text-xl"}
                        text-txtSubHeader hover:text-txtHeader transition-colors duration-200`}
            href={href}
        >
            {content}
        </Link>
